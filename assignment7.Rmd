...
df %>% ggplot()+
  geom_bar(mapping=aes(x=Sex)) + facet_wrap(~Survived)
```

2. Make a plot using `facet_wrap` by two categorical variables (`facet_wrap(~variable1+variable2)`)
```{r}
df %>% ggplot()+ 
  geom_point(mapping=aes(x=Age, y=Fare)) + facet_wrap(~Survived)
```
...
df %>% 
  ...
  summarise(mean.fare = Fare) %>% 
  ggplot()+
  geom_col(aes(x=Sex, y=mean.fare, fill=Survived), position = 'dodge')
```
...
gg <- df %>% 
  ...
  ggplot()+
  geom_col(aes(x=Sex, y=mean.fare, fill=Survived), position = 'dodge')
ggsave(filename = 'thing.png', plot = gg)
```
...
```{r}
#Asia: Hong Kong, Japan, Nepal, South Korea
df %>% 
  filter(location=='Hong Kong'|location=='Japan'|location=='Nepal'|location=='South Korea') %>% 
  group_by(date,location) %>%
...
df %>% 
  filter(location %in% c('Argentina','Chile','Ecuador','Peru','Uruguay') & date > '2020-12-31' & date < '2022-01-01') %>%
  group_by(date,location) %>%
  summarise(total_vac2=sum(total_vaccinations, na.rm=TRUE)) %>%
  ggplot()+
  geom_point(mapping = aes(x = date, 
